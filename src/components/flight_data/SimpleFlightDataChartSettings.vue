...
<v-table density="compact" v-if="widgetData.graphSeriesSetting">
    <thead>
      <tr>
        <th class="text-left">
          Series
    ...
    <tbody>
      <tr
        v-for="item, key in widgetData.graphSeriesSetting"
        :key="key"
      >
        <td> {{ key }} </td>
        <td><v-checkbox density="compact" v-model="item.enabled" ></v-checkbox></td>
        <td><v-checkbox density="compact" v-model="item.minMaxEnabled" :disabled="!item.enabled"></v-checkbox></td>
      </tr>
    </tbody>
  </v-table>
