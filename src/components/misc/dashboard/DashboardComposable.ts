...
export type StoreObject =  {matrix: WidgetMatrix, widgets: Widget[], canDrop: boolean, rows: number, cols: number}

export type ResizeDirections = 'shrinkHorizontal' | 'enlargeHorizontal' | 'shrinkVertical' | 'enlargeVertical'

const dashboardStores: {[id: string]: Ref<StoreObject>} = {}

/**
 * A central store for a dashboard componentds
 */
export function useDashboardStore(dashboardId: string){

    if(!dashboardStores[dashboardId])
        dashboardStores[dashboardId] = ref({matrix: [], widgets: [], canDrop: false, rows: 0, cols: 0})

    const store = dashboardStores[dashboardId]
